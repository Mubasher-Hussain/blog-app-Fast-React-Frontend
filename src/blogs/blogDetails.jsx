...
export function BlogDetails({match}) {
  ...
  useEffect(async() => {
    ...
    let isAuth = '';
    if (localStorage.getItem('username') == blogData.data.author_username)
      isAuth = 'yes'
    setBlogDetails({ post: blogData.data, comment_list: commentData.data, isAuth: isAuth })
  ...
  function displayDetail(){
    if (blogDetails && blogDetails.post){
      return (
        <div>
          <p>Author: <NavLink to={'../blogsList/' + blogDetails.post.author_username} >{blogDetails.post.author_username}</NavLink></p>
          <p>Title: {blogDetails.post.title}</p>
          <p>Content: { blogDetails.post.content }</p>
          <p>Modified At: { blogDetails.post.modified_at }</p>
        ...
        <div class="w-100">
          <div class="d-flex justify-content-between align-items-center">
            <div class="d-flex flex-row align-items-center">
              <span class="mr-2">{params.comment.commentator_username}</span>
            </div>
